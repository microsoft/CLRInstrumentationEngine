...
1. Module load 
    a. This is used to add new metadata to the assembly
    b. Parameters: 
        i. Module info (Assembly name, strong name, path,  Appdomain)
        ii. IMetadataImport / IMetadataExport
2. Before Jit Complete
    a. This is the primary callback where IL rewriting occurs
    b. Parameters
        i. Method Info
        ii. IL Graph
3. On Rejit
    a. This occurs the next time a specific function is called after a rejit request is made and like Before Jit Complete, is used to rewrite method il. 
    b. Parameters
        i. Method Info
        ii. IL Graph
4. OnClassLoad
    a. Intellitrace uses this callback which occurs every time a new class loads to update the type system. This callback MUST be optional as it incurs high overhead 
    b. Parameters:
        i. Method Info
        ii. Class Info?
        iii. IMetadataImport
5. Function Enter / Function Exit 
    a. These callbacks are issued every time a function is called and every time a function returns. Intellitrace and Production BPs don’t use these, but Tofino does. Note that these also incur large runtime overhead and use a naked calling convention making it difficult to multiplex
6. Inline tracking
    a. This allows components to track where a method has been inlined. Intellitrace turns inlining off globally today. Tofino allows inlining to occur. 
    b. Inlining greatly complicates the way the profiler works. However, the increase in performance is likely worth building something into the high level structures to make it easy to go from a metadata token to its inline sites to allow consumers to easily rejit all of the parent function to undo inlining.
