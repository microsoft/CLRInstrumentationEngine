...
#else
#define ATLMFC_CRT_ERRORCHECK(expr) (expr)
#endif
...
inline void __cdecl wmemcpy_s(
	_Out_writes_to_(_N1,_N) WCHAR *_S1,
	_In_ size_t _N1,
	_In_reads_(_N) const WCHAR *_S2,
	_In_ size_t _N)
{
...
inline void __cdecl wcscpy_s(
	_Out_writes_z_(_S1max) WCHAR *_S1,
	_In_ size_t _S1max,
	_In_z_ const WCHAR *_S2)
{
...
inline errno_t __cdecl wcsncpy_s(
	_Out_writes_z_(_SizeInChars) WCHAR *_Dest,
	_In_ size_t _SizeInChars,
	_In_z_ const WCHAR *_Source,
	_In_ size_t _Count)
{
...
inline void __cdecl wcscat_s(
	_Inout_updates_z_(_SizeInChars) WCHAR * _Dst,
	_In_ size_t _SizeInChars,
	_In_z_ const WCHAR * _Src)
{
...
inline void __cdecl wcslwr_s(
	_Inout_updates_z_(_SizeInChars) WCHAR * _Str,
	_In_ size_t _SizeInChars)
{
...
inline void __cdecl ultow_s(
	_In_ unsigned long _Val,
	_Out_writes_z_(_SizeInChars) WCHAR *_Buf,
	_In_ size_t _SizeInChars,
	_In_ int _Radix)
{
...
inline size_t __cdecl wcsnlen(
	_In_reads_z_(_Maxsize) const WCHAR *_Wcs,
	_In_ size_t _Maxsize)
{
	return ::PAL_wcsnlen(_Wcs, _Maxsize);
}
