...
class CComQIPtrElementTraits :
	public CDefaultElementTraits< ATL::CComQIPtr< I > >
{
public:
	typedef I* INARGTYPE;
};
...
class CInterfaceArray :
	public CAtlArray< ATL::CComQIPtr< I >, CComQIPtrElementTraits< I > >
{
public:
...
class CInterfaceList :
	public CAtlList< ATL::CComQIPtr< I >, CComQIPtrElementTraits< I > >
{
public:
	CInterfaceList(_In_ UINT nBlockSize = 10) throw() :
		CAtlList< ATL::CComQIPtr< I >, CComQIPtrElementTraits< I > >( nBlockSize )
	{
	}
...
UINT PickAtlHashMapSize(UINT nBinsEstimate);
...
UINT CAtlMap< K, V, KTraits, VTraits >::PickSize(_In_ size_t nElements) const throw()
{
    ...
    return PickAtlHashMapSize(nBinsEstimate);
}
...
public:
	// TOOD: investigate
	// For some reason, it doesn't work in clang. Add these explicitly here.
	typedef typename KTraits::INARGTYPE KINARGTYPE;
	typedef typename KTraits::OUTARGTYPE KOUTARGTYPE;
	typedef typename VTraits::INARGTYPE VINARGTYPE;
	typedef typename VTraits::OUTARGTYPE VOUTARGTYPE;
	typedef class CRBTree<K, V, KTraits, VTraits> CPair_t;
	...
	const typename CRBTree< K, V, KTraits, VTraits >::CPair* Lookup(/* _In_ */ KINARGTYPE key) const throw();
	typename CRBTree< K, V, KTraits, VTraits >::CPair*  Lookup(/* _In_ */ KINARGTYPE key) throw();
	POSITION SetAt(
		/* _In_ */ KINARGTYPE key,
		/* _In_ */ VINARGTYPE value) throw( ... );
	bool RemoveKey(/* _In_ */ KINARGTYPE key) throw();
...
template< typename K, typename V, class KTraits, class VTraits >
const typename CRBTree< K, V, KTraits, VTraits >::CPair* CRBMap< K, V, KTraits, VTraits >::Lookup(
	/* _In_ */ KINARGTYPE key) const throw()
{
...
template< typename K, typename V, class KTraits, class VTraits >
typename CRBTree< K, V, KTraits, VTraits >::CPair* CRBMap< K, V, KTraits, VTraits >::Lookup(
	/* _In_ */ KINARGTYPE key) throw()
{
	return this->Find(key);
}
...
bool CRBMap< K, V, KTraits, VTraits >::Lookup(
	...
	_Out_ VOUTARGTYPE value) const throw( ... )
{
	const auto pLookup = this->Find( key );
	if( pLookup == NULL )
		return false;
...
public:
	...
	const typename CRBTree<K, V, KTraits, VTraits>::CPair* GetNextWithKey(
		_Inout_ POSITION& pos,
		/* _In_ */ KINARGTYPE key) const throw();
	typename CRBTree<K, V, KTraits, VTraits>::CPair* GetNextWithKey(
		_Inout_ POSITION& pos,
		/* _In_ */ KINARGTYPE key) throw();
...
template< typename K, typename V, class KTraits, class VTraits >
const typename CRBTree< K, V, KTraits, VTraits >::CPair* CRBMultiMap< K, V, KTraits, VTraits >::GetNextWithKey(
	_Inout_ POSITION& pos,
	/* _In_ */ KINARGTYPE key) const throw()
{
	ATLASSERT( pos != NULL );
...
template< typename K, typename V, class KTraits, class VTraits >
typename CRBTree< K, V, KTraits, VTraits >::CPair* CRBMultiMap< K, V, KTraits, VTraits >::GetNextWithKey(
	_Inout_ POSITION& pos,
	/* _In_ */ KINARGTYPE key) throw()
{
	ATLASSERT( pos != NULL );
	auto pNode = this->GetNext( pos );
	if( (pos == NULL) || !KTraits::CompareElements( static_cast<typename CRBTree<K, V, KTraits, VTraits>::CPair* >( pos )->m_key, key ) )
	{
		pos = NULL;
	}
