...
inline int _mbstowcsz(
	_Out_writes_(count) WCHAR* wcstr,
	_In_z_ const char* mbstr,
	_In_ ULONG count)
{
	// count is number of WCHAR's
...
#pragma warning (pop)
	...
	static int __cdecl SafeStringLen(_In_opt_z_ LPCWSTR psz) throw()
	{
		...
		return (psz != NULL) ? int( PAL_wcslen( psz ) ) : 0;
	}

	static int __cdecl GetCharLen(_In_opt_z_ const WCHAR* pch) throw()
	{
		(void)pch;
		// returns char length
		return 1;
...
template< typename _CharType = WCHAR >
class ChTraitsCRT :
	public ChTraitsBase< _CharType >
#else
template<>
class ChTraitsCRT< WCHAR > :
...
public:
	...
	static int __cdecl IsDigit(_In_ WCHAR ch) throw()
	{
        // TODO:
        /*
	...
	static int __cdecl IsSpace(_In_ WCHAR ch) throw()
	{
        // TODO:
        /*
...
public:
	...
	CStringT(_In_ SystemString __gc* pString) :
		CThisSimpleString( StringTraits::GetDefaultManager() )
	{
		const WCHAR __pin* psz = PtrToStringChars( pString );
		*this = psz;
	}
...
//all methods in user build (see afxstr.h), so need to include the methods in MFC dll builds.
	...
	CSTRING_EXPLICIT CStringT(_In_opt_z_ WCHAR* pszSrc) :
		CThisSimpleString( StringTraits::GetDefaultManager() )
	{
		const WCHAR *psz = reinterpret_cast< const WCHAR* >( pszSrc );
		*this = psz;
	}
#endif
...
public:
		...
		CThisSimpleString( StringTraits::GetDefaultManager() )
	{
		...
		if( nLength > 0 )
		{
			...
			StringTraits::ConvertToBaseType( pszBuffer, nDestLength + 1, pch, nLength );
	...
	CStringT& operator=(_In_opt_z_ PCYSTR pszSrc)
	{
		...
		{
			PXSTR pszBuffer = this->GetBuffer( nDestLength );
			StringTraits::ConvertToBaseType( pszBuffer, nDestLength + 1, pszSrc);
			this->ReleaseBufferSetLength( nDestLength );
		}
		else
		{
...
public:
		...
		_In_ int nCount = 1)
	{
		...
		if( nCount > 0 )
		{
			...
			PXSTR pszBuffer = CSimpleStringT< BaseType, _CSTRING_IMPL_::_MFCDLLTraitsCheck<BaseType, StringTraits>::c_bIsMFCDLLTraits >::GetBuffer();
			Checked::memmove_s( pszBuffer+iIndex, nXCHARsToCopy*sizeof( XCHAR ),
				pszBuffer+iIndex+nCount, nXCHARsToCopy*sizeof( XCHAR ) );
			CSimpleStringT< BaseType, _CSTRING_IMPL_::_MFCDLLTraitsCheck<BaseType, StringTraits>::c_bIsMFCDLLTraits >::ReleaseBufferSetLength( nNewLength );
		}
		...
		_In_z_ PCXSTR pszNew)
	{
		...
		if( nCount > 0 )
		{
			...
			PXSTR pszBuffer = this->GetBuffer( max( nNewLength, nOldLength ) );
		...
		_Inout_ int& iStart) const
	{
		...
		else
		{
			...
			PCXSTR pszEnd = this->GetString()+GetLength();
			if( pszPlace < pszEnd )
			{
		...
		_In_ va_list args)
	{
		...
		CSimpleStringT< BaseType, _CSTRING_IMPL_::_MFCDLLTraitsCheck<BaseType, StringTraits>::c_bIsMFCDLLTraits >::ReleaseBufferSetLength( nCurrentLength+nAppendLength );
	}
		...
		_In_ va_list args)
	{
		...
		PXSTR pszBuffer = CSimpleStringT< BaseType, _CSTRING_IMPL_::_MFCDLLTraitsCheck<BaseType, StringTraits>::c_bIsMFCDLLTraits >::GetBuffer( nLength );
		StringTraits::Format( pszBuffer, nLength+1, pszFormat, args );
		CSimpleStringT< BaseType, _CSTRING_IMPL_::_MFCDLLTraitsCheck<BaseType, StringTraits>::c_bIsMFCDLLTraits >::ReleaseBufferSetLength( nLength );
	}
...
public:
		...
		m_pMgr( pMgr )
	{
		...
		*static_cast< WCHAR* >( m_pData->data() ) = 0;
	}
...
public:
	...
	CFixedStringT< StringType, t_nChars >& operator=(_In_z_ const WCHAR* psz)
	{
		StringType::operator=( psz );
		return *this;
	}
