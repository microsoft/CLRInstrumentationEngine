...
#pragma once
// Don't use thread locale in linux.
...
private:
	void Init(_In_z_ LPCWSTR psz) throw(...)
	{
		...
		int nLength = static_cast<int>(PAL_wcslen( psz ))+1;
		AtlConvAllocMemory(&m_psz,nLength,m_szBuffer,t_nBufferLength);
		ATLASSUME(m_psz != NULL);
		Checked::memcpy_s( m_psz, nLength*sizeof( WCHAR ), psz, nLength*sizeof( WCHAR ));
	}

public:
	LPWSTR m_psz;
	WCHAR m_szBuffer[t_nBufferLength];
...
private:
		...
		_In_ UINT nConvertCodePage) throw(...)
	{
		...
		int nLengthW = static_cast<int>(PAL_wcslen( psz ))+1;
...
#define W2A(lpw) (\
	((_lpw = lpw) == NULL) ? NULL : (\
		(_convert = (static_cast<int>(PAL_wcslen(_lpw))+1), \
...
#define W2A_EX(lpw, nChars) (\
	((_lpw_ex = lpw) == NULL) ? NULL : (\
		_convert_ex = (static_cast<int>(PAL_wcslen(_lpw_ex))+1),\
		FAILED(::ATL::AtlMultiply(&_convert_ex, _convert_ex, static_cast<int>(sizeof(WCHAR)))) ? NULL : \
		ATLW2AHELPER(	\
...
{ 
	...
	return static_cast<int>(PAL_wcslen(x));
}
