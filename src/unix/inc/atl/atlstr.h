...
namespace ATL
{
    ...
    public:
        ...
        // Reverse the string - disabled as wcsrv is missing from the PAL
...
#pragma warning(pop)
        ...
        static int GetBaseTypeLength(_In_z_ const WCHAR* pszSrc) throw()
        {
            // Returns required buffer length in XCHARs
        ...
        static int GetBaseTypeLength(
            _In_reads_(nLength) const WCHAR* pszSrc,
            _In_ int nLength) throw()
        {
            // Returns required buffer length in XCHARs
        ...
        static void ConvertToBaseType(
            _Out_writes_(nDestLength) _CharType* pszDest,
            _In_ int nDestLength,
            _In_reads_(nSrcLength) const WCHAR* pszSrc,
            _In_ int nSrcLength = -1) throw()
        {
            // nLen is in XCHARs
        ...
        static int SafeStringLen(_In_opt_z_ const WCHAR* psz) throw()
        {
            // returns length in wchar_ts
            return (psz != NULL) ? static_cast<int>(PAL_wcslen(psz)) : 0;
        }
    ...
    template<>
    class ChTraitsOS< WCHAR > :
        public ChTraitsBase< WCHAR >
    {
    protected:
    ...
    public:
        static int tclen(_In_opt_z_ const WCHAR*) throw()
        {
            return 1;
        }
        ...
        _Ret_maybenull_z_ static WCHAR* _strrev(_Inout_opt_z_ WCHAR* psz) throw()
        {
            // Optimize NULL, zero-length, and single-char case.		
        ...
        static int strspn(
            ...
            _In_z_ const WCHAR* pszCharSet) throw()
        {
            int nRet = 0;
            const WCHAR* p = psz;
            while (*p != 0)
        ...
        static WCHAR* CharNext(_In_ const WCHAR* p) throw()
        {
            return AtlCharNext(p);
        }

        static int StringCompare(
            _In_z_ const WCHAR* pstrOne,
            _In_z_ const WCHAR* pstrOther) throw()
        {
            return PAL_wcscmp(pstrOne, pstrOther);
        }
        ...
        static int StringSpanIncluding(
            _In_z_ const WCHAR* pszBlock,
            _In_z_ const WCHAR* pszSet) throw()
        {
        ...
        _Ret_z_ static WCHAR* StringUppercase(
            _Inout_updates_z_(size) WCHAR* psz,
            _In_ size_t size) throw()
        {
            if (size > (UINT_MAX / sizeof(WCHAR)))
            {
                // API only allows DWORD size
                AtlThrow(E_INVALIDARG);
            }
        ...
        _Ret_z_ static WCHAR* StringLowercase(
            _Inout_updates_z_(size) WCHAR* psz,
            _In_ size_t size) throw()
        {
            if (size > (UINT_MAX / sizeof(WCHAR)))
        ...
        // Reverse the string - disabled as wcsrv is missing from the PAL
        //_Ret_maybenull_z_ static WCHAR* StringReverse(_Inout_z_ WCHAR* psz) throw()
        //{
        //    return _strrev(psz);
        //}
...
#pragma warning(pop)
        ...
        static int Format(
            _Out_writes_z_(nLength) WCHAR* pszBuffer,
            _In_ size_t nLength,
            _In_z_ _Printf_format_string_ const WCHAR* pszFormat,
            _In_ va_list args)
        {
...
#pragma warning(pop)
        ...
        static void ConvertToBaseType(
            _Out_writes_(nDestLength) WCHAR* pszDest,
            _In_ int nDestLength,
            _In_reads_(nSrcLength) const char* pszSrc,
            _In_ int nSrcLength = -1) throw()
        {
        ...
        //static void ConvertToOem(_In_opt_z_ WCHAR*)
        //{
        //    ATLASSERT(FALSE);
        //}

        // this conversion on Unicode strings makes no sense
        //static void ConvertToAnsi(_In_opt_z_ WCHAR*)
        //{
        //    ATLASSERT(FALSE);
        //}
        ...
        static void FloodCharacters(
            _In_ WCHAR ch,
            _In_ int nLength,
            _Out_writes_all_(nLength) WCHAR* pstr) throw()
        {
            // nLength is in XCHARs
        ...
        static _Ret_maybenull_z_ BSTR AllocSysString(
            _In_reads_(nDataLength) const WCHAR* pchData,
            _In_ int nDataLength) throw()
        {
        ...
        static DWORD FormatMessage(
            ...
            _In_ DWORD dwMessageID,
            _In_ DWORD dwLanguageID,
            _Out_writes_z_(nSize) WCHAR* pstrBuffer,
            _In_ DWORD nSize,
            _In_opt_ va_list* pArguments) throw()
        {
        ...
        ATLPREFAST_SUPPRESS(6103)
            ...
            static DWORD GetEnvironmentVariable(
                _In_z_ const WCHAR* pstrVar,
                _Out_writes_opt_z_(dwSize) WCHAR* pstrBuffer,
                _In_ DWORD dwSize) throw()
        {
    ...
    inline LPWSTR __cdecl ChTraitsCRT<WCHAR>::StringUppercase(
        _Inout_updates_(size) LPWSTR psz,
        _In_ size_t size) throw()
    {
        return ChTraitsOS<WCHAR>::StringUppercase(psz, size);
    }

    inline LPWSTR __cdecl ChTraitsCRT<WCHAR>::StringLowercase(
        _Inout_updates_(size) LPWSTR psz,
        _In_ size_t size) throw()
    {
        return ChTraitsOS<WCHAR>::StringLowercase(psz, size);
    }
...
#error _ATL_CSTRING_NO_CRT flag is not compatible with the current WINAPI_FAMILY
    ...
    typedef CStringT< WCHAR, StrTraitATL< WCHAR, ChTraitsCRT< WCHAR > > > CAtlStringW;
    ...
    typedef CStringT< WCHAR, StrTraitATL< WCHAR > > CAtlStringW;
