...
// IDispatch specific stuff
        ...
        _In_ VARIANT* pVar) throw()
    {
        ...
        if (V_VT(pVar) == VT_UNKNOWN || V_VT(pVar) == VT_DISPATCH || 
            (V_VT(pVar) & VT_ARRAY) || (V_VT(pVar) & VT_BYREF))
        {
...
public:
    ...
    CComQIPtr(_Inout_ const CComQIPtr<T>& lp) throw() :
        CComPtr<T>(lp.p)
    {
    }
    ...
    T* operator=(_Inout_ const CComQIPtr<T>& lp) throw()
    {
        if(*this!=lp)
        {
            CComQIPtr(lp).Swap(*this);
...
public:
    ...
    CComQIPtr(_Inout_ const CComQIPtr<IUnknown>& lp) throw() :
    ...
    IUnknown* operator=(_Inout_ const CComQIPtr<IUnknown>& lp) throw()
    {
        if(*this!=lp)
        {
            CComQIPtr(lp).Swap(*this);
...
typedef CComQIPtr<IDispatch> CComDispatchDriver;
...
#define ATL_NO_CCOMBSTR_ADDRESS_OF_ASSERT
    ...
    /* Commented out because there is no VarBstrCat implementation in the coreclr pal,
    * and we don't yet need this function.
    ...
    */
    ...
    _Check_return_ HRESULT Append(_In_ WCHAR ch) throw()
    {
        return( Append( &ch, 1 ) );
    }
