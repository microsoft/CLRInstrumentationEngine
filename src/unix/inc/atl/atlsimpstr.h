...
#define _SW( psz ) ATL::CStaticString< WCHAR, sizeof( L##psz ) >( L##psz )
...
public:
	...
	CSimpleStringT& operator+=(_In_ WCHAR ch)
	{
		AppendChar( XCHAR( ch ) );
...
public:
	...
	static int __cdecl StringLength(_In_opt_z_ const WCHAR* psz) throw()
	{
		if( psz == NULL )
		{
			return( 0 );
		}
		return( int( PAL_wcslen( psz ) ) );
	}
	...
	static int __cdecl StringLengthN(
		_In_reads_opt_z_(sizeInXChar) const WCHAR* psz,
		_In_ size_t sizeInXChar) throw()
	{
		if( psz == NULL )
		{
		...
		return( int( PAL_wcsnlen( psz, sizeInXChar ) ) );
	}
...
typedef CSimpleStringT< WCHAR > CSimpleStringW;
typedef CStrBufT< TCHAR > CStrBuf;
typedef CStrBufT< char > CStrBufA;
typedef CStrBufT< WCHAR > CStrBufW;
